...
var toDisplayString = (val) => {
  return val == null ? "" : isArray2(val) || isObject2(val) && (val.toString === objectToString2 || !isFunction2(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
...
var isReservedProp2 = makeMap2(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted");
...
function mergeObjectOptions(to, from) {
  return to ? extend2(extend2(Object.create(null), to), from) : from;
}
function mergeWatchOptions(to, from) {
  ...
  if (!from)
    return to;
  const merged = extend2(Object.create(null), to);
  for (const key in from) {
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (!(instance.type.__hmrId || instance.parent && instance.parent.type.__hmrId) && (optimized || patchFlag > 0) && !(patchFlag & 16)) {
    ...
    if (attrs !== rawCurrentProps) {
      for (const key in attrs) {
        if (!rawProps || !hasOwn2(rawProps, key)) {
          delete attrs[key];
          hasAttrsChanged = true;
        }
      }
...
var isBuiltInDirective = makeMap2("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo");
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    for (const key in toMerge) {
      if (key === "class") {
        ...
        if (existing !== incoming && !(isArray2(existing) && existing.includes(incoming))) {
...
var publicPropertiesMap = extend2(Object.create(null), {
  $: (i) => i,
  $el: (i) => i.vnode.el,
  $data: (i) => i.data,
...
var PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if (publicGetter) {
      ...
      if (data !== EMPTY_OBJ2 && (key[0] === "$" || key[0] === "_") && hasOwn2(data, key)) {
...
var isReservedProp3 = makeMap3(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted");
